...
import edu.eci.arsw.beatApp.persistence.CarRepository;
...
import edu.eci.arsw.beatApp.services.DriverServices;
...
public class DriverServicesImpl implements DriverServices {
    ...
    @Override
    public List<Driver> getAllDrivers() throws BeatAppApplicationPersistenceException {
    ...
    @Override
    public void saveDriver(Driver driver) throws BeatAppApplicationPersistenceException {
        Optional<Driver> optionalDriver = driverRepository.findByEmail(driver.getEmail());
        if (optionalDriver.isPresent()) {
            throw new BeatAppApplicationPersistenceException(BeatAppApplicationPersistenceException.DRIVER_ALREDY_EXISTS);
        } else {
    ...
    @Override
    public Driver findDriverByEmail(String driver) throws BeatAppApplicationPersistenceException {
        Optional<Driver> optinalDriver = driverRepository.findByEmail(driver);
        if (!optinalDriver.isPresent())
            throw new BeatAppApplicationPersistenceException(BeatAppApplicationPersistenceException.DRIVER_NOT_FOUND);
        return optinalDriver.get();
    }

    
    @Override
    public List<Car> findCarsByEmail(String user) throws BeatAppApplicationPersistenceException {
    ...
    @Override
    public void updateDriver(String user, Driver driver) throws BeatAppApplicationPersistenceException {
    ...
    private void setCars(Driver driver, List<Car> cars) throws BeatAppApplicationPersistenceException{
        Boolean flag = false;
            ...
            for (Car i: cars){
                for (Car j: currentCars){
                    if (j.getPlate().equals(i.getPlate())){
                        flag = true;
                        throw new BeatAppApplicationPersistenceException(BeatAppApplicationPersistenceException.CAR_ALREDY_EXISTS);
                    }
                }
                if(!flag){
                    i.setDriver(driver);
    ...
    @Override
    public void updateCar(String user, Car car) throws BeatAppApplicationPersistenceException {
        Optional<Driver> optinalDriver = driverRepository.findByEmail(user);
        boolean present = optinalDriver.isPresent();
        if (!present){
            throw new BeatAppApplicationPersistenceException(BeatAppApplicationPersistenceException.DRIVER_NOT_FOUND);
